...
// AI-THINKER HARDWARE BUTTON MAPPINGS
#define KEY_VOL_UP         18
...
bool last_key_up = HIGH;
bool last_key_play = HIGH;
...
void init_es8388_codec() {
    ...
    write_reg(0x0A, 0xBB); 
    write_reg(0x0C, 0x4C); 
    Serial.println("AI-Thinker ES8388 Initialized.");
}
...
<head>
    ...
    <title>BioTech | Neural Audio Sync</title>
    <style>
        ...
        .log-box { background: #000; border: 1px dashed #00eaff; padding: 10px; margin-top: 20px; font-family: monospace; font-size: 12px; height: 60px; overflow-y: scroll; text-align: left; }
    </style>
</head>
<body>
    <div class="card">
        ...
        <p style="margin-top:20px; font-size:12px;">HARDWARE TRIGGER LOG:</p>
        <div class="log-box" id="log">Awaiting hardware inputs...</div>
    </div>
<script>
    let ws, audioCtx;
    let nextPlayTime = 0;
    ...
    function startAudioContext() {
        ...
        audioCtx.resume().then(() => { setupWebSocket(); });
    }
    ...
    function setupWebSocket() {
        ...
        ws.onmessage = (event) => {
            // Check if packet is text (Hardware Button Command) or binary (Audio Stream)
            if (typeof event.data === "string") {
                let logBox = document.getElementById('log');
                logBox.innerHTML += "<br>> " + event.data;
...
void setup() {
    ...
    // Set up onboard tactile buttons as inputs with internal pullups
    pinMode(KEY_VOL_UP, INPUT_PULLUP);
    pinMode(KEY_PLAY_PAUSE, INPUT_PULLUP);
...
void loop() {
    ...
    // 1. READ OBOARD PHYSICAL BUTTONS & TRIGGER WEB EVENT
    if (ws.count() > 0) {
        bool current_key_up = digitalRead(KEY_VOL_UP);
        bool current_key_play = digitalRead(KEY_PLAY_PAUSE);
        
        // Check for state changes (button presses draw low to GND)
        if (current_key_up == LOW && last_key_up == HIGH) {
            ws.textAll("TRIGGER: KEY_UP_PRESSED (Action Command)");
            delay(50); // Simple debounce
        }
        if (current_key_play == LOW && last_key_play == HIGH) {
            ws.textAll("TRIGGER: PLAY_PAUSE_PRESSED (Interact Command)");
            delay(50); 
        }
        
        last_key_up = current_key_up;
        last_key_play = current_key_play;
    }

    // 2. PROCESS STREAM AS NORMAL
